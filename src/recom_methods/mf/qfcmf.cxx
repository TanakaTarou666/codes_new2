...
QFCMF::QFCMF(int missing_count)
    : QFCRecom(missing_count),
      ...
      prev_user_factors_(),
      prev_item_factors_() {
    method_name_ = append_current_time_if_test("QFCMF");
}

void QFCMF::set_parameters(double latent_dimension_percentage, int cluster_size, double fuzzifier_em, double fuzzifier_lambda, double reg_parameter,
...
void QFCMF::set_initial_values(int seed) {
    seed *= 1000000;
    std::mt19937_64 mt;
    for (int c = 0; c < cluster_size_; c++) {
...
double QFCMF::calculate_objective_value() {
    double result = 0;
...
bool QFCMF::calculate_convergence_criterion() {
    bool result = false;
#if defined ARTIFICIALITY
...
void QFCMF::calculate_prediction() {
