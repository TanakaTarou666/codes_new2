...
FMWithALS::FMWithALS(int missing_count)
    : FMBase(missing_count), Recom(missing_count), w0_(), prev_w0_(), w_(), prev_w_(), v_(), prev_v_(), e_(), q_(), x_(), transpose_x_() {
...
void FMWithALS::set_initial_values(int seed) {
    seed *= 1000000;
    w0_ = 0.0;
    w_ = Vector(sum_users_items_, 0.0, "all");
    v_ = Matrix(latent_dimension_, sum_users_items_);
    ...
    transpose_x_ = SparseMatrix(sum_users_items_, sparse_missing_data_.nnz(), sparse_missing_data_.nnz() * 2);
...
void FMWithALS::calculate_factors() {
    ...
    for (int a = 0; a < sum_users_items_; ++a) {
        ...
        for (int b = 0; b < transpose_x_.nnz(a); b++) {
            double tmp_e = e_[transpose_x_.dense_index(a, b)];
            numerator_w += (tmp_e - w_[a]);
            denominator_w += 1;
        }
        double wa = -numerator_w / (denominator_w + reg_parameter_ / 2);
        for (int b = 0; b < transpose_x_.nnz(a); b++) {
            e_[transpose_x_.dense_index(a, b)] += wa - w_[a];
        }
        w_[a] = wa;
    }
    ...
    for (int f = 0; f < latent_dimension_; ++f) {
        ...
        for (int a = 0; a < sum_users_items_; ++a) {
            ...
            for (int b = 0; b < transpose_x_.nnz(a); b++) {
                ...
                numerator_v += (e_[tmp_dense_index] - tmp_v[a] * tmp_h) * tmp_h;
                denominator_v += tmp_h * tmp_h;
            }
            double va = -numerator_v / (denominator_v + reg_parameter_ / 2);
            for (int b = 0; b < transpose_x_.nnz(a); b++) {
                int tmp_dense_index = transpose_x_.dense_index(a, b);
                e_[tmp_dense_index] += (va - tmp_v[a]) * h_value[tmp_dense_index];
                q_(f, tmp_dense_index) += (va - tmp_v[a]);
            }
            v_(f, a) = va;
        }
    }
    ...
    //                 tmp_x = x_(i, j);
    //                 int tmp_x_dense_index = tmp_x.dense_index(a);
    //                 h_value[l] = -tmp_v[tmp_x_dense_index] + tmp_q[l];
    //                 l++;
    //             }
    //         }
    ...
    //                 q_(f, l) += va[tmp_x_dense_index] - tmp_v[tmp_x_dense_index];
    //                 l++;
    //             }
    //         }
    //     }
